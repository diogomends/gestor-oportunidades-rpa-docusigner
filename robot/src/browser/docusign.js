...
export function normalizeEnvelopeStatus(rawText) {
  ...
  if (clean.includes("concluido") || clean.includes("concluído") || clean === "completed") {
    return { status: "completed", rawStatus: rawText, unknown_status: false };
  }

  if (clean.includes("aguardando") || clean === "waiting_others") {
    return { status: "waiting_others", rawStatus: rawText, unknown_status: false };
  }

  if (clean.includes("anulado") || clean === "voided") {
    return { status: "voided", rawStatus: rawText, unknown_status: false };
  }

  if (clean.includes("falha na entrega") || clean.includes("falha") || clean === "delivery_failed") {
    return { status: "delivery_failed", rawStatus: rawText, unknown_status: false };
  }
...
export async function extractEnvelopesFromCurrentPage(page, repName = "") {
  ...
  const rows = await page.$$(selectors.agreements?.row || "[data-qa='manage-envelopes-list.table'] tr");
  const envelopes = [];
  ...
  for (const row of rows) {
    ...
    const subjectEl =
      (await row.$("[data-qa$='-subject']")) ||
      (await row.$("a[data-qa*='envelope-']")) ||
      (await row.$("a"));
    const subject = subjectEl ? (await subjectEl.innerText()).trim() : "";
    const href = subjectEl && typeof subjectEl.getAttribute === "function" ? await subjectEl.getAttribute("href") : "";
...
export async function fetchAgreementsByRepresentative(page, options = {}) {
  ...
  const { buildAgreementsUrl } = await import("./selectors.js");
