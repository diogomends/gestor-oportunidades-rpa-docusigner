...
- **REQ-006 (Captura de Envelope ID)**: Quando o envio for concluído, o robô DEVE tentar capturar o `envelopeId` através de cascata resiliente de 4 níveis: (1) URL atual pós-redirecionamento; (2) Interceptação de requisições de rede durante a submissão (`attachNetworkEnvelopeInterceptor`); (3) Busca e leitura ativa no DOM da tabela/listagem de documentos (`waitForSelector`); (4) Fallback do `existingEnvelopeId` válido do job.
