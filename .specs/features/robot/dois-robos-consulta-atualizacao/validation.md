...
**Commit**: `db3b594` + docs (T11)
**Gate**: `npm test` 152 backend + 56 robot pass, `npm run build:robot -- --role=query` artifact check pending (build gate manual)
...
| AC-01.7 | `PATCH /instance/job/:jobId/status` com `result.envelopes` executa reconciliação batch | PASS | `controllers/robotInstanceController.js:598-648` Zod array + `syncContractStatus` + `Contract.find` ativos + `mapEnvelopeStatusToContractStatus` + auto-download; `contractSyncService` fallback Mongoose |
...
- `npm run test:backend` — 152 pass (incl. `RobotJob` conditional, `statusSyncScheduler` dual-robot guard, `getMetrics` readyState)
- `npm run test:robot` — 56 pass (browser, imap, roundcube, selectors)
- `npm test` — 208 pass total (152+56)
- Build gate — `build.js` matriz `ROBOT_API_KEY_N × role` validada via `parseArgs` + `define ROBOT_ROLE`; `dist/robot-query-N/` e `dist/robot-update-N/` artefatos a validar em `npm run build:robot` manual (chave necessária)
