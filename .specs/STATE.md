...
- **Shim do Módulo Inspector no Pipeline de Build do Robô (.exe)**: Adicionado patch automático pós-cópia em `robot/build/build.js` que substitui o require inicial do `inspector` em `playwright-core/lib/coreBundle.js` por um objeto shim seguro com classes e métodos vazios (`Session`, `url`), prevenindo a exceção `ERR_INSPECTOR_NOT_AVAILABLE` disparada no ambiente empacotado pelo `@yao-pkg/pkg`.
...
- **Padronização e Ajuste do Botão de Confirmação MFA (verify-code / Verify)**: Atualizados os seletores em `robot/src/browser/selectors.js`, `backend/src/modules/robot-docusign/selectors/docusign-ui.json`, `backend/src/modules/robot-docusign/services/robotSelectors.js` e a submissão em `backend/src/modules/robot-docusign/services/robotSession.js` para incluir suporte explícito a `button[data-qa='verify-code']`, `button:has-text('Verify')` e `[data-qa='verify-code']`, além de priorizar o seletor dedicado de confirmação MFA no login do servidor com fallback para teclado/submit.
