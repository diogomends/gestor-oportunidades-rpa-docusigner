...
>
> Contratos entram na fila do robô quando **elegíveis**: `status != "rascunho"` + `documents[].originalUrl` não-vazio + e-mail do destinatário (`client.representante.email` | `signer.email` | `email` | `clientEmail`) e com a flag `operations.send !== false`. Filtro centralizado em `backend/src/modules/robot-docusign/utils/contractEligibility.js` (`GERADO_ELIGIBLE_FILTER`, `CONTRACT_ELIGIBLE_FILTER`, `isEligibleForSend`) é aplicado em `GET /instance/next-job` (criação + pós-validação com revert `em_processamento_robot`→status original), no `robotScheduler` (fallback Mongoose + pós-filtro da API `gestorApiClient`) e no `robot/job-runner.js` (validação pré-browser antes de `chromium.launch`).
